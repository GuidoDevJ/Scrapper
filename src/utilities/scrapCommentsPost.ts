import { InstagramPostDetails } from '../types/types';
  ...
  try {
    commentsDivs = await page.evaluate(() => {
      ...
      if (ownerCommentsContainer1) {
        ownerCommentsContainer = ownerCommentsContainer1;
      } else if (ownerCommentsContainer2) {
        ownerCommentsContainer = ownerCommentsContainer2;
      }
      if (ownerCommentsContainer) {
        let allComments = Array.from(ownerCommentsContainer.children);
        allComments = ownerCommentsContainer1
          ? allComments.slice(2)
          : allComments;
        const allCom = allComments.map((commentElement) => {
